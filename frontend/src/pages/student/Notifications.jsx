...
const Notifications = () => {
  ...
  // Icon by type - Purple Text
  const getIcon = (type) => {
    switch (type) {
      case "job": return <Briefcase size={14} className="text-purple-500" />;
      case "internship": return <Building size={14} className="text-purple-500" />;
      case "course": return <BookOpen size={14} className="text-purple-500" />;
      default: return <Info size={14} className="text-purple-500" />;
    }
  };

  const getIconBg = (type) => {
    switch (type) {
      case "job": return "bg-purple-50 border-purple-100";
      case "internship": return "bg-purple-50 border-purple-100";
      case "course": return "bg-purple-50 border-purple-100";
  ...
  return (
    // ✅ Light Background - Internship Style
    <div className="bg-gray-50 min-h-screen flex flex-col">
      <Navbar />

      <div className="max-w-2xl mx-auto px-4 py-6 w-full">

        {/* Header - Internship Style */}
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Notifications</h1>
            {unreadCount > 0 && (
              <p className="text-xs text-purple-600 mt-0.5">{unreadCount} unread</p>
            )}
          </div>
          {unreadCount > 0 && (
            <button
              onClick={handleMarkAllRead}
              className="flex items-center gap-1.5 text-xs font-medium text-purple-600 bg-purple-50 border border-purple-200 px-3 py-1.5 rounded-lg hover:bg-purple-100 transition"
            >
        ...
        {/* Empty State - Internship Style */}
        {notifications.length === 0 ? (
          <div className="text-center mt-24">
            <Bell size={40} className="mx-auto mb-3 text-purple-300" />
            <p className="text-purple-700 font-medium">No notifications yet</p>
            <p className="text-purple-400 text-sm mt-1">You're all caught up!</p>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {notifications.map((notif) => (
              ...
              >
                {/* Icon - Light Theme */}
                <div className={`w-8 h-8 rounded-lg border flex items-center justify-center shrink-0 mt-0.5 ${getIconBg(notif.type)}`}>
                  {getIcon(notif.type)}
                </div>
