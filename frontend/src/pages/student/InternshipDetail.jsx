...
const InternshipDetail = () => {
  ...
  const { singleInternship } = useSelector((s) => s.internship);
  ...
  useEffect(() => {
    setIsApplied(
      singleInternship?.applications?.some((app) => app === user?._id) || false
    );
  }, [singleInternship, user?._id]);
  ...
  const handleApply = async () => {
    ...
    try {
      const res = await axios.post(
        `${INTERNSHIP_API_END_POINT}/apply/${internshipId}`,
        {},
    ...
    <div className="min-h-screen bg-[#f8f9fa]">
      ...
      <div className="bg-white border-b sticky top-0 z-30">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <h1 className="text-sm font-medium text-gray-700">Internship details</h1>
        </div>
      </div>
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-xl border p-5">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                ...
                {isPPO && (
                  <span className="text-xs px-2.5 py-1 rounded bg-amber-50 text-amber-700 font-medium border border-amber-100">
                    PPO available
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 mt-5 pt-5 border-t">
                <div>
                  <p className="text-[11px] text-gray-500 mb-0.5 flex items-center gap-1">
                    <IndianRupee size={11} /> Stipend
                  </p>
                  <p className="text-sm font-semibold text-gray-900">₹{stipend}/mo</p>
                </div>
                <div>
                  <p className="text-[11px] text-gray-500 mb-0.5 flex items-center gap-1">
                    <Users size={11} /> Openings
                  </p>
                  <p className="text-sm font-semibold text-gray-900">{openings || 'Multiple'}</p>
                </div>
                <div>
                  <p className="text-[11px] text-gray-500 mb-0.5 flex items-center gap-1">
            ...
            {/* About */}
            <div className="bg-white rounded-xl border p-5">
              <h3 className="text-sm font-semibold text-gray-900 mb-3">About the internship</h3>
              ...
              <div className="space-y-3 text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-auto font-medium text-gray-900">
                    {applyBy ? new Date(applyBy).toLocaleDateString() : 'Open'}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <Tag size={13} className="text-gray-400" />
                  <span className="text-gray-500">Category</span>
                  <span className="ml-auto font-medium text-gray-900">{category || 'General'}</span>
                </div>
              </div>
            </div>
