...
import { Search, Sparkles, ArrowRight, Briefcase, Users } from "lucide-react";

const Hero = () => {
  const navigate = useNavigate();

  return (
    <section className='min-h-screen bg-gradient-to-br from-indigo-900 via-blue-900 to-indigo-800 px-4 py-6 flex flex-col items-center justify-center relative overflow-hidden'>
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto text-center">
        {/* Mini Badge */}
        <div className="inline-flex items-center gap-1.5 bg-white/10 backdrop-blur-sm border border-white/20 px-3 py-1 rounded-full mb-3">
          <Sparkles size={12} className="text-yellow-300" />
          <span className="text-[10px] font-medium text-blue-100">#1 Career Platform</span>
        </div>

        {/* Compact Heading */}
        <h1 className="text-2xl font-bold leading-tight text-white mb-1.5">
          Learn. Experience.
          <br />
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className="flex-1 bg-gradient-to-r from-yellow-400 to-orange-400 text-indigo-900 px-4 py-2 rounded-lg font-bold text-xs hover:shadow-lg hover:shadow-yellow-400/30 transition-all flex items-center justify-center gap-1"
          >
            Jobs
            <ArrowRight size={12} />
          </button>
          <button
            ...
            className="flex-1 bg-white/10 backdrop-blur-sm text-white px-4 py-2 rounded-lg font-bold text-xs border border-white/20 hover:bg-white/20 transition-all"
          >
            Courses
          </button>
        </div>
        ...
        {/* Tiny Search Bar */}
        <div className="bg-white/95 backdrop-blur-sm rounded-xl p-1.5 flex items-center gap-1.5 mb-3 shadow-xl">
          <div className="flex items-center gap-1 flex-1 px-2">
            <Search size={14} className="text-gray-400" />
            <input
              type="text"
              placeholder="Search jobs..."
              className="w-full py-1.5 text-xs text-gray-800 outline-none bg-transparent"
            />
          </div>
          <button
            onClick={() => navigate('/jobs')}
            className="bg-indigo-600 px-3 py-1.5 rounded-lg text-[10px] font-semibold text-white whitespace-nowrap hover:bg-indigo-700 transition-all"
          >
            Search
          </button>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-1 mb-5">
          {["Remote", "MNC", "Fresher", "Data Science"].map((tag) => (
            <button 
              key={tag} 
              className="bg-white/10 backdrop-blur-sm border border-white/10 px-2 py-0.5 rounded-full text-[9px] text-blue-100 hover:bg-white hover:text-indigo-900 transition-all"
            >
              {tag}
            </button>
          ))}
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl px-3 py-2.5 border border-white/10">
            <Briefcase size={12} className="text-blue-300 mx-auto mb-0.5" />
            <h2 className="text-sm font-bold text-yellow-400">12K+</h2>
            <p className="text-[8px] text-blue-300">Live Jobs</p>
          </div>
          <div className="bg-white/10 backdrop-blur-sm rounded-xl px-3 py-2.5 border border-white/10">
            <Users size={12} className="text-blue-300 mx-auto mb-0.5" />
            <h2 className="text-sm font-bold text-yellow-400">1.2M+</h2>
            <p className="text-[8px] text-blue-300">Job Seekers</p>
          </div>
        </div>

        {/* Micro Footer */}
        <p className="text-[8px] text-blue-300/60 mt-3">
